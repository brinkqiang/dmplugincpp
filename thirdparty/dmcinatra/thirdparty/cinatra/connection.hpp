...
namespace cinatra {
    ...
    public:
        ...
        void reset_timer() {
            ...
            timer_.async_wait([self](asio::error_code const& ec) {
                if (ec) {
                    return;
                }
        ...
        void write_ranges_data(std::string&& buf, bool eof) {
            ...
            asio::async_write(
                socket(), asio::buffer(chunked_header_),
                [this, self, eof](const asio::error_code& ec, size_t) {
                if (ec) {
                    return;
                }
    ...
    private:
        ...
        void handle_pipeline(size_t ret, std::size_t bytes_transferred) {
            ...
            asio::async_write(
                ...
                &rep_str](const asio::error_code& ec,
                    std::size_t bytes_transferred) {
                rep_str.clear();
            ...
            multipart_parser_.on_part_begin = [this](const multipart_headers& headers) {
                ...
                if (is_multi_part_file_) {
                    ...
                    try {
                        ...
                        req_.open_upload_file(name, filename);
                    }
                    catch (const std::exception& ex) {
                        req_.set_state(data_proc_state::data_error);
        ...
        void do_read_multipart() {
            ...
            asio::async_read(
                socket(), asio::buffer(req_.buffer(), req_.left_body_len()),
                [self, this](asio::error_code ec, std::size_t length) {
                if (ec) {
                    req_.set_state(data_proc_state::data_error);
                    call_back();
                    response_back(status_type::bad_request, "mutipart error");
        ...
        void ws_ping() {
            ...
            timer_.async_wait(
                [self = this->shared_from_this()](asio::error_code const& ec) {
                if (ec) {
                    self->close(false);
                    return;
                }
        ...
        void handle_chunked_header(const asio::error_code& ec) {
            if (ec) {
                return;
            }
        ...
        std::function<void(const asio::error_code&)> send_failed_cb_ =
            nullptr;
