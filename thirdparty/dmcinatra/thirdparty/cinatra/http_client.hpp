...
namespace cinatra {
    ...
    public:
        ...
        response_data request(http_method method, std::string uri, req_content_type type = req_content_type::json, size_t seconds = 15, std::string body = "") {
            ...
            if (status == std::future_status::timeout) {
                promise_ = nullptr;
                return { asio::error::make_error_code(asio::error::basic_errors::invalid_argument), 404, REQUEST_TIMEOUT };
            }
        ...
        void async_request_impl(http_method method, std::string uri, callback_t cb, req_content_type type = req_content_type::json, size_t seconds = 15, std::string body = "") {
            ...
            if (in_progress_) {
                set_error_value(cb, asio::error::basic_errors::in_progress, MULTIPLE_REQUEST);
                return;
            }
            else {
                in_progress_ = true;
            ...
            if (method != http_method::POST && !body.empty()) {
                set_error_value(cb, asio::error::basic_errors::invalid_argument, METHOD_ERROR);
                return;
            }
            ...
            if (!r) {
                set_error_value(cb, asio::error::basic_errors::invalid_argument, INVALID_URI);
                return;
            }
        ...
        void download_impl(std::string src_file, std::string dest_file, int64_t size, callback_t cb, size_t seconds = 60) {
            ...
            if (code) {
                cb_ = std::move(cb);
                callback(asio::error::make_error_code(asio::error::basic_errors::invalid_argument), INVALID_FILE_PATH);
                return;
            }
            ...
            if (!download_file_->is_open()) {
                cb_ = std::move(cb);
                callback(asio::error::make_error_code(asio::error::basic_errors::invalid_argument), OPEN_FAILED);
                return;
            }
        ...
        void download(std::string src_file, std::function<void(asio::error_code, std::string_view)> chunk, size_t seconds = 60) {
    ...
    private:
        ...
        void callback(const asio::error_code& ec, std::string error_msg) {
            callback(ec, 404, std::move(error_msg));
        }

        void callback(const asio::error_code& ec, int status) {
            callback(ec, status, "");
        }

        void callback(const asio::error_code& ec, int status, std::string_view result) {
        ...
        void async_connect(context ctx) {
            ...
            (asio::error_code ec, const asio::ip::tcp::resolver::iterator& it) {
                if (ec) {
                    callback(ec);
                    return;
                }

                asio::async_connect(socket_, it, [this, self = shared_from_this(), ctx = std::move(ctx)]
                (asio::error_code ec, const asio::ip::tcp::resolver::iterator&) {
                    cancel_timer();
                    if (!ec) {
                        has_connected_ = true;
                        if (is_ssl()) {
        ...
        void send_msg(const context& ctx) {
            ...
            async_write(write_msg_, [this, self = shared_from_this()](const asio::error_code& ec, const size_t length) {
                if (ec) {
                    callback(ec);
                    close();
                    return;
        ...
        void send_multipart_msg(const context& ctx) {
            ...
            if (ec || start_ == -1) {
                file->close();
                callback(asio::error::make_error_code(asio::error::basic_errors::invalid_argument), FILE_SIZE_ERROR);
                return;
            }
        ...
        void handshake(context ctx) {
            ...
            ssl_stream_->async_handshake(asio::ssl::stream_base::client,
                [this, self, ctx = std::move(ctx)](const asio::error_code& ec) {
                if (!ec) {
                    do_read_write(ctx);
                }
                else {
        ...
        void do_read() {
            ...
            async_read_until(TWO_CRCF, [this, self = shared_from_this()](auto ec, size_t size) {
                ...
                if (!ec) {
                    ...
                    if (ret < 0) {
                        callback(asio::error::make_error_code(asio::error::basic_errors::invalid_argument), 404,
                            RESP_PARSE_ERROR);
                        if (buf_size > size) {
        ...
        void async_read(size_t size_to_read, Handler handler) {
            if (is_ssl()) {
#ifdef CINATRA_ENABLE_SSL
                asio::async_read(*ssl_stream_, read_buf_, asio::transfer_exactly(size_to_read), std::move(handler));
#endif
            }
            else {
                asio::async_read(socket_, read_buf_, asio::transfer_exactly(size_to_read), std::move(handler));
            }
        }
        ...
        void async_read_until(const std::string& delim, Handler handler) {
            if (is_ssl()) {
#ifdef CINATRA_ENABLE_SSL
                asio::async_read_until(*ssl_stream_, read_buf_, delim, std::move(handler));
#endif
            }
            else {
                asio::async_read_until(socket_, read_buf_, delim, std::move(handler));
            }
        }
        ...
        void async_write(const std::string& msg, Handler handler) {
            if (is_ssl()) {
#ifdef CINATRA_ENABLE_SSL
                asio::async_write(*ssl_stream_, asio::buffer(msg), std::move(handler));
#endif
            }
            else {
                asio::async_write(socket_, asio::buffer(msg), std::move(handler));
            }
        }
        ...
        void send_file_data(std::shared_ptr<std::ifstream> file) {
            ...
            auto self = this->shared_from_this();
            async_write(multipart_str_, [this, self, file = std::move(file)](asio::error_code ec, std::size_t length) mutable {
                if (!ec) {
        ...
        void set_error_value(const callback_t& cb, const asio::error::basic_errors& ec, const std::string& error_msg) {
            if (promise_) {
                promise_->set_value({ asio::error::make_error_code(asio::error::basic_errors::invalid_argument), 404, error_msg });
            }
            if (cb) {
                cb({ asio::error::make_error_code(asio::error::basic_errors::invalid_argument), 404, error_msg });
            }
            read_close_finished_ = {};
        }
    ...
    private:
        ...
        std::function<void(asio::error_code, std::string_view)> on_chunk_ = nullptr;
