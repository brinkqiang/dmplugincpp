...
namespace cinatra {
    ...
    public:
        async_client(asio::io_service& ios) : ios_(ios), resolver_(ios), socket_(ios){
        }
    ...
    private:
        void async_connect(const std::string& host, const std::string& port, std::weak_ptr<std::promise<bool>> weak) {
            ...
            (asio::error_code ec, const asio::ip::tcp::resolver::iterator& it) {
                ...
                asio::async_connect(socket_, it, [this, self = shared_from_this(), weak]
        ...
        void handshake(std::weak_ptr<std::promise<bool>> weak) {
            ...
            ssl_stream_->async_handshake(asio::ssl::stream_base::client,
                [this, self, weak](const asio::error_code& ec) {
                if (!ec) {
                    do_read();
                }
                else {
        ...
        void write() {
            auto& msg = outbox_[0];
            async_write(msg, [this, self = shared_from_this()](const asio::error_code& ec, const size_t length) {
                if (ec) {
                    //print(ec);
                    close();
                    return;
...
#ifdef CINATRA_ENABLE_SSL
        void upgrade_to_ssl(std::function<void(asio::ssl::context&)> ssl_context_callback) {
            if (ssl_stream_)
                return;
