    ...
    public:
        ...
        void upgrade_to_websocket(const request& req, response& res) {
            ...
            if (!protocal_str.empty()) {
                res.add_header("Sec-WebSocket-Protocol", { protocal_str.data(), protocal_str.length() });
            }
        }
        ...
        */
        ...
        std::vector<asio::const_buffer> format_message(const char* src, size_t length, opcode code) {
            size_t header_length = encode_header(length, code);
            return{ asio::buffer(msg_header_, header_length), asio::buffer(src, length) };
        }
        ...
        size_t left_header_len() const {
            return left_header_len_;
        }
